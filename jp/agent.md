...
インフラストラクチャのプロビジョニングは **Terraform のみ** で管理され、このリポジトリの **スコープ外** です。
...
### 2. インフラストラクチャプロビジョニングコードを含めない

- Terraform, CDK, CloudFormation, または AWS SDK ベースのリソース作成は行いません。
- このリポジトリには **アプリケーションコードのみ** が含まれます。
- インフラストラクチャは Terraform を介して別途定義および適用されます。
...
- インフラストラクチャまたはプロビジョニングコードの追加を避ける
