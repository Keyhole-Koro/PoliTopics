...
LocalStack の補足: テストや開発前に `source scripts/export_test_env.sh` を実行すると主要なローカルデフォルトが一括で設定されます。
...
## 環境変数
- ヒント: `source scripts/export_test_env.sh` を実行すると、LocalStack 用の主要なデフォルト値を一括で設定できる。
...
- Recap: `GEMINI_API_KEY`, `DISCORD_WEBHOOK_ERROR`, `DISCORD_WEBHOOK_WARN`, `DISCORD_WEBHOOK_BATCH`, `R2_WRITE_ENDPOINT_URL`, `R2_REGION` (デフォルト `auto`), `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_ARTICLE_BUCKET`, `R2_PUBLIC_ASSET_URL`, `ENABLE_NOTIFICATION`, `NOTIFICATION_DELAY_MS`。
