...
export DATA_COLLECTION_BUILD_CMD="cd \"$DATA_COLLECTION_DIR\" && pnpm install && pnpm run build"
...
export RECAP_BUILD_CMD="cd \"$RECAP_DIR\" && pnpm install && pnpm run build:local"
...
export WEB_BUILD_CMD="npm --prefix \"$WEB_DIR\" run build:backend:local"
