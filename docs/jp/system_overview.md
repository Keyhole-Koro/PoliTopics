...
- 役割: 国会会議録を取得し、LLM 用のプロンプトに分割し、アセットを S3 に保存して、DynamoDB にタスクを登録する。
- メインフロー:
  - 国会 API から会議録を取得。
  - 長いテキストを LLM サイズのチャンクに分割。
  - プロンプト JSON を S3 に保存し、タスクに S3 URL を記録。
...
- メインフロー:
  ...
  - S3 からプロンプトをロードし、Map/Reduce 要約を実行。
  - 重い記事アセットを S3 に保存。
...
- メインフロー:
  - frontend: 検索と記事詳細 UI のための Next.js SPA。
  - backend: DynamoDB をクエリし、S3 からアセットを取得する Fastify + Lambda API。
...
4. Recap が記事を DynamoDB に書き込み、アセットを S3 に保存する。
5. Web backend が DynamoDB + S3 から記事をフロントエンドに提供する。
...
- 主要アイテムタイプ:
  - 記事メインアイテム
    ...
    - `SK`: `META`
    - `type`: `ARTICLE`
    - `asset_url`: 詳細アセット JSON への S3 ポインタ (API によって署名付き URL として返される)
    ...
    - `summary`, `soft_language_summary`, `middle_summary`, `dialogs` は S3 に保存され、`asset_url` 経由で参照される。
  - シンインデックスアイテム (リスト/検索)
...
## ノート: S3
- DataCollection: プロンプトと中間結果を保存 (`prompt_url`, `result_url`)。
- Recap/Web: 詳細な記事アセットを保存; DynamoDB は `asset_url` のみを保持。
