...
- チャンク化タスクでは、各チャンクの order 範囲（`chunks[].based_on_orders`）を保持し、reduce 入力へ渡して復元/検証精度を高める。
- プロンプト/リデュース結果は S3 に保存し、記事の重いアセットは R2 に保存する。
- 記事のメタデータを DynamoDB に、重いアセットを R2 に永続化する。
- 対話要約は必要な分の箇条書きで簡潔にし、長くなる場合は「主張」「説明」「質問」「回答」「根拠」「影響」「次の対応」の固定見出しで整理する。UI向けに `summary_sections` / `soft_language_sections`（title + bullets 配列）を追加してよい。summary / セクション / QA の内容は重複させない。やさしい言い換えは短文で作成する。
...
- 要約本文には `[[orders:1,2,3]]` のようなタグを含め、該当発言へジャンプ/強調表示できる。
...
- Recap は処理前にタスクフィールドを検証する:
  ...
  - `chunks[].based_on_orders` は任意だが推奨。未設定時は chunk 出力や URL パターンからフォールバック推定する。
