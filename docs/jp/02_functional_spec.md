...
- プロンプトペイロードを S3 に保存し、タスクを DynamoDB に作成する。
...
- リデュース結果を S3 に保存する。
- 記事のメタデータを DynamoDB に、重いアセットを S3 に永続化する。
...
- ユーザーが入力すると、`/search/suggest` から提案が取得される。
...
- Web `/search` は `words`, `categories`, `houses`, `meetings` にカンマ区切りのリストを使用する。
- Web `/search/suggest` の制限はデフォルトで 5。
...
- Recap は処理前にタスクフィールドを検証する:
  ...
  - チャンク化されたタスクは、S3 URL を含むチャンク定義を含まなければならない。
- DataCollection は日付範囲の形式を `YYYY-MM-DD` として検証する。

## ロールベースの動作
- ロールベースの UI または API アクセスは実装されていない。
- DataCollection `/run` は `x-api-key` ヘッダーによってのみ保護されている。
